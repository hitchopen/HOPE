...
fleet layer（保持老板基线不变）
  bridge + monitor + assert-only E-stop
        ...
        ├──────────────────────────────┐
        ▼                              ▼
Runner observer（只读）             Runner command proxy（attended control）
...
fleet layer 始终保持通用、可回退和急停-only。Runner layer 是单独安装的 trial profile；第一阶段
只发布 `/hope/**`，正好被现有 bridge 的 `/hope/.*` topic allowlist 覆盖，不需要放宽任何
写权限。
...
### P1：Runner 只读 observer（nightly_built 仓库实现已完成，待部署）

`foxglove/a3/hope_observer.py`：
...
- 只发布标准消息到 `/hope/**`，没有应用层 ROS service、parameter mutation 或 client
  publisher。observer 关闭 parameter services；Jazzy 自动生成的 type-description service 不在
  bridge allowlist 中，因此不能从 Foxglove 调用。

配套 layout 是 `foxglove/layouts/model21800_observer.json`。这些状态只用于 operator 和日志
...
- Laptop mocap/world/calibration/base relay、HDU Planner、MDU SHADOW 的 start/stop/status；
- `RefreshXHit` 复用已有 request/status 文件；
- evidence manifest 和日志收集状态。

动作返回“已接受/失败 + 当前状态 + audit ID”，但不形成控制 gate。现场顺序是先进入
PD_STAND 并站稳，再执行独立的 `Calibration` 和 `Refresh x_hit`；完成后才由操作员决定是否
进入 MOTION。不会新增 READY、stable-frame、confidence、station、source-age 或平衡门控。
...
- `/hope/calibrate` (`std_srvs/Trigger`)：十 marker 重算 `P1 -> pelvis_link`，
  保存静止时的 `world -> pelvis_link` audit snapshot，并等待新 receipt 生效；
- `/hope/refresh_x_hit` (`std_srvs/Trigger`)：独立刷新 Planner `x_hit`；
- 后端先要求本次 Runner 处于新鲜 `PD_STAND`，再调用 Laptop 十-marker 校准服务；
- 新 calibration receipt 必须出现在新鲜 schema-2 base packet；
- x_hit 按钮从固定 session/attempt/PID 文件解析当前 C++ Planner并独立刷新；
...
只精确 allowlist 七个 Runner `Trigger` service、`Calibration`、`Refresh x_hit` 和 assert-only E-stop。
完整冻结合同见
[`foxglove_runner_control_contract.md`](foxglove_runner_control_contract.md)。
...
- fleet config 永久保留急停-only；Runner control config 以后单独 opt-in；
- 不提供软件急停远程解除；
- 不让 A3 访问 Motive 网段；
- P1 telemetry 不是 readiness gate，也不是接触、落点或真机平衡证明；
- 每个新增 unit 都必须可单独 disable/remove，不修改 vendor unit 文件；
- 任一 Runner layer 异常时，可停用新增 unit 并回到当前 runbook 的 SSH/TTY 流程。
...
当前没有宣称 live `Calibration`、厂商 E-stop、有电机 PD_STAND/MOTION、Ready-to-Serve、Serve
