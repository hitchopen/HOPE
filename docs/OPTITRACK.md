...
The production A3 path uses all ten waist markers once at the start of every
run and does not require an independent pelvis tracker. After the MDU runner
has entered and settled in PD_STAND, Foxglove's
`/hope/control/enter_prepare` workflow asks the external computer to run
`p1_marker_cad_calibrator` on `/optitrack/rigid_body_markers`. It recomputes on
every PREPARE, even if the previous run's JSON is present.
Motive supplies
...
current-run calibration. The same calculation can be run manually on the
external computer, but only after PD_STAND has already been established by the
approved robot procedure:
...
the JSON. Policy entry remains locked until the base-pose packet carries the
SHA-derived ID of the current PREPARE receipt.
