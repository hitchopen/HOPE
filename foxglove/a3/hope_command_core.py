"""Narrow file-contract helpers for explicit Runner Foxglove actions."""
...
from hope_observer_core import DecodeError, XHitStatus, parse_x_hit_status


REQUEST_ID_PATTERN = re.compile(r"[0-9]{1,32}")
CALIBRATION_SHA_PATTERN = re.compile(r"[0-9a-f]{64}")
...
def parse_calibration_receipt_sha(message: str) -> str:
    """Return the exact SHA emitted by the laptop calibration service."""

    value = str(message).strip()
    if CALIBRATION_SHA_PATTERN.fullmatch(value) is None:
        raise DecodeError("calibration service did not return a 64-hex SHA")
    return value


def calibration_receipt_id(receipt_sha: str) -> int:
    """Map a receipt SHA to the exact u52 carried by base schema 2."""

    return int(parse_calibration_receipt_sha(receipt_sha)[:13], 16)
