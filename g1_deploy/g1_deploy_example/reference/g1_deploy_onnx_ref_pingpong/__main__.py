...
"""CLI entrypoint: run the G1 reference runner against the MuJoCo sim.

    python -m g1_deploy_onnx_ref_pingpong --view --realtime --model-xml /path/g1_pingpong.xml
...
swings. Point ``--onnx`` at your exported ``hope_pingpong_g1.onnx`` and ``--model-xml``
at your G1 MJCF (a G1 ping-pong MJCF is not shipped yet — see the runtime YAML TODO).
...
def build_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(prog="g1_deploy_onnx_ref_pingpong", description=__doc__)
    ...
    p.add_argument("--model-xml", type=Path, default=None,
                   help="override the G1 ping-pong MJCF path (mujoco backend)")
...
def main(argv: list[str] | None = None) -> int:
    ...
    if not Path(cfg.onnx_path).is_file():
        print(
            ...
            f"(hope_pingpong_g1.onnx) and pass --onnx, or set policy.onnx_path.",
            file=sys.stderr,
        )
        return 2
