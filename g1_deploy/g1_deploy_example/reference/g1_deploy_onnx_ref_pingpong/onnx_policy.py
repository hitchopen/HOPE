...
"""ONNX actor wrapper (Unitree G1).

The exported policy is a single-input, single-output network:

    observation[1, 105]  ->  raw_action[1, 29]

No observation normalization is applied (the observation is raw). The G1 has no
passive neck, so the applied action equals ``raw_action``; it is fed back as the
next tick's ``last_action`` and passed through the ActionAdapter.
"""
...
class OnnxPolicy:
        ...
        if embedded:
            ...
            if embedded_names != tuple(JOINT_NAMES):
                raise ValueError(
                    ...
                    "joints in the canonical order (joint_order_unitree_g1.yaml)."
                )
    ...
    def infer(self, obs: np.ndarray) -> np.ndarray:
        """Run one forward pass. Returns ``raw_action`` as ``float32`` length 29."""
